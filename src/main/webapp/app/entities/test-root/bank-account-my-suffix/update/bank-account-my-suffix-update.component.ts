...
export class BankAccountMySuffixUpdateComponent implements OnInit {
    ...
    this.dataUtils.loadFileToForm(event, this.editForm, field, isImage).subscribe({
      error: (err: FileLoadError) =>
        this.eventManager.broadcast(
          new EventWithContent<AlertError>('sampleWebfluxMariaApp.error', { ...err, key: 'error.file.' + err.key })
        ),
    });
  }
  ...
  protected updateForm(bankAccount: IBankAccountMySuffix): void {
    this.editForm.patchValue({
      ...
      meanQueueDuration: bankAccount.meanQueueDuration,
  ...
  protected createFromForm(): IBankAccountMySuffix {
    return {
      ...
      meanQueueDuration: this.editForm.get(['meanQueueDuration'])!.value,
